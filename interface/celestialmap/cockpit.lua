...
require "/celestialmap/cockpitview.lua"
require "/celestialmap/cockpitutil.lua"
...
function init()
  ...
  if celestial.skyInHyperspace() and celestial.currentSystem() then
...
function lockDestination()
  local isCurrent = compare(self.currentSystem, celestial.currentSystem())

  local dest
  if self.selection.planet then
    dest = "CelestialWorld:" .. self.selection.planet["location"][1] .. ":" .. self.selection.planet["location"][2] .. ":" .. self.selection.planet["location"][3] .. ":" .. self.selection.planet["planet"] .. ":" .. self.selection.planet["satellite"]-- .. "=0.1100"
    destName = celestial.planetName(self.selection.planet)
  elseif self.selection.object then
    objType = self.selection.object.typeName
    if self.selection.object.typeName == "anomaly" then
      objType = "spaceencounter"
    end
    dest = "InstanceWorld:" .. objType .. ":" .. self.selection.object.uuid
    sb.logInfo(sb.print(celestial.objectParameters(self.selection.object.uuid)))
    if celestial.objectParameters(self.selection.object.uuid) == nil then
      destName = "Remote " .. self.selection.object.typeName
    elseif celestial.objectParameters(self.selection.object.uuid) ~= nil then
      destName = celestial.objectParameters(self.selection.object.uuid).displayName
    end
  end
  status.setStatusProperty("warpDestination", dest)
  status.setStatusProperty("destinationName", destName)
...
function systemScreenState(system, warpIn)
  ...
  self.mappedObjects = player.mappedObjects(system)
  ...
  while true do
    ...
    for _,click in pairs(takeInputEvents()) do
      ...
      if down then
        ...
        if button == 2 then
          if false then--canFlyShip(system) then
            pane.playSound(self.sounds.startTravel)
    ...
    if selection and selection[1] == "coordinate" and self.viewSelection then
      ...
      if isCurrent and compare(self.travel.system, system.location) then
        if self.travel.target then
          if self.travel.target[1] ~= "coordinate" or celestial.visitableParameters(self.travel.target[2]) then
            ...
            sb.logInfo(sb.print(self.travel.target))

            if self.travel.target[1] == "orbit" then
              dest = "CelestialWorld:" .. self.travel.target[2]["target"]["location"][1] .. ":" .. self.travel.target[2]["target"]["location"][2] .. ":" .. self.travel.target[2]["target"]["location"][3] .. ":" .. self.travel.target[2]["target"]["planet"] .. ":" .. self.travel.target[2]["target"]["satellite"]
            elseif self.travel.target[1] == "object" then
              local objType = celestial.objectType(self.travel.target[2])
              if objType == "anomaly" then
                objType = "spaceencounter"
              end

              dest = "InstanceWorld:" .. objType .. ":" .. self.travel.target[2]
            else
              dest = "CelestialWorld:" .. self.travel.target[2]["location"][1] .. ":" .. self.travel.target[2]["location"][2] .. ":" .. self.travel.target[2]["location"][3] .. ":" .. self.travel.target[2]["planet"] .. ":" .. self.travel.target[2]["satellite"]
            end
            --status.setStatusProperty("warpDestination", dest)
    ...
    if selection and selection[1] == "object" then
      ...
      if celestial.objectTypeConfig(typeName) then
    ...
    for _,click in pairs(takeInputEvents()) do
      ...
      if down then
        ...
        if button == 2 and hover then
          if false then--canFlyShip(system) and locationVisitable(hover) then
            pane.playSound(self.sounds.startTravel)
